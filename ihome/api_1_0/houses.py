...
def save_house_info():
    ...
    area_id = house_data.get("area_id")  # 房屋所在区域编号
...
def get_house_list():
    ...
    if page <= total_page:
        # 设置缓存数据
        redis_key = "house_%s_%s_%s_%s" % (start_date, end_date, area_id, sort_key)  # 设置redis 字典类型的键
        # 哈希类型
        try:
            # redis_store.hset(redis_key, page, resp_json)  # redis字典类型
            ...
            # 开启多条记录
            pipline.multi()
            pipline.hset(redis_key, page, resp_json)  # redis字典类型
